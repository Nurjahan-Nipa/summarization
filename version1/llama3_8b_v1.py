...
# ========== 1. Load LLaMA 3 7B and Tokenizer ==========
llm_model_id = "neuralmagic/Meta-Llama-3-7B-Instruct-FP8"
llm = LLM(model=llm_model_id, tensor_parallel_size=1)
...
for item in dataset:
    ...
    ground_truth = " ".join([s["sentence"] for s in sentences if s["truth"] == 1])
...
with open("assortis_llama3_7b.csv", "w", newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(["Question ID", "Title", "Answer Body", "Abstractive Summary", "Extractive Summary", "Ground Truth"])
    writer.writerows(csv_rows)

print("✅ Summarization completed. Saved to assortis_llama3_7b.csv")
