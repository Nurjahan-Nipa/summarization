...
output_csv = "llama8b_fp8_fewshot.csv"
...
few_shot_prompt = ""
for ex in dataset[:2]:  # use first 3 items as few-shot examples
    q = ex["question_title"]
    a = ex["answer_body"]
...
for item in dataset[3:]:  # skip first 3 used for few-shot
    qid = item["question_id"]
    title = item["question_title"]
    answer = item["answer_body"]
    sentences = item["sentences"]

    prompt = few_shot_prompt + f"### Example\nQuestion: {title}\nAnswer: {answer}\nSummary:"
    ...
    extractive = " ".join(
        s["sentence"] for s in sentences if entails(abstractive, s["sentence"])
    )
...
with open(stats_file, "a", newline='') as f:
    ...
    writer.writerow([
        ...
        f"{sum(rouge1)/len(rouge1):.4f}",
        f"{sum(rouge2)/len(rouge2):.4f}",
        f"{sum(rougel)/len(rougel):.4f}",
        f"{F1.mean().item():.4f}",
        f"{sum(bleu)/len(bleu):.4f}"
    ])
